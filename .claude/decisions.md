...
## 十、合规与内容安全（2026-07-28 公安备案完成后新增）

### 语音输入自动填表功能 —— 弃做
- **决定**：不做。物品借还表单维持手动填写。
- **理由**：非必需功能，但牵扯要素过多——STT 选型、LLM 抽取、数据出境合规、降级链、
  API 额度——投入产出比低。
- **若将来重启**：底层 STT/LLM **必须用境内 API**（通义千问 / 阿里云 NLS），用户数据
  不出境，以规避 PIPL 个人信息出境的安全评估 / 单独同意流程。纯学习目的可在**本地**用
  Ollama 跑小模型（学习沙盒，不接生产、不碰合规）。

### UGC 内容监管（《网络安全法》第 47 条）
- **轻量本地敏感词过滤**：`server/services/content-filter.js`，接入 `addItem` /
  `editItem`（itemDetail）与 `register`（username）三处；命中返回 400 `sensitive-content`。
- 定位：把项目从"零过滤"提升到"已履行合理注意义务"。词库为起步版，政治/暴恐类应以官方
  名单补全；生产级鉴黄鉴政可接**阿里云内容安全**（境内，不涉出境）。

### 个人信息保护（PIPL）
- **隐私政策页** `/privacy`（`src/components/PrivacyPage/`）+ **注册同意勾选**（未勾不提交）。
- 内容简短写实：收集什么 / 用途 / 存储与安全 / 用户权利 / 联系方式。
- ⚠️ 待办：`privacy.s5Body` 的联系方式为占位符 `[你的联系邮箱]`，需填真实邮箱。

### 实名制（《网络安全法》第 24 条，灰区）—— 暂不做
- **决定**：维持"用户名+密码"，不加手机号/实名认证。
- **理由**：发布性质弱（物品仅对借还双方可见）；引入手机号反而新增一类敏感个人信息与合规
  负担。风险敞口主要由敏感词过滤 + 隐私政策覆盖。

### 日志留存（《网络安全法》第 21 条，≥6 个月）
- 轮转 + 留存 ~200 天方案见 `deploy/LOGGING.md`：**pm2-logrotate 为主**，系统
  logrotate（`deploy/logrotate/pnr`，`copytruncate`）为备选，二选一。
- 应用层脱敏已达标（`lib/logger.js`）。

### 数据出境
- **无**。语音功能弃做后，项目不涉及任何个人信息出境。

### XSS —— 现状达标
- 三层防护齐全：React 自动转义 + 全项目零 `innerHTML`/`dangerouslySetInnerHTML` +
  session cookie `HttpOnly` + `helmet` CSP。
- helmet 现用 `useDefaults` CSP，可选收紧（禁 `unsafe-inline`），**暂不做**。

### 品牌图标
- favicon 由默认 `vite.svg` 改为**原创 `public/favicon.svg`**（P&R 白色衬线 + 主题绿
  `#2d5016`，复刻已有 PWA 图标风格），删除 `vite.svg`。
- 说明：图标设计已存在（PWA 图标），故未按第八节派 uiux agent 另行设计，直接复刻统一。
